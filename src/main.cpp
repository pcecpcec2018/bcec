...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 18);      // "standard" scrypt target limit for proof of work, results with 0,000244140625 proof-of-work difficulty
CBigNum bnProofOfStakeLimit(~uint256(0) >> 20);
CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 16);

unsigned int nTargetSpacing     = 60;               // 60 seconds
unsigned int nStakeMinAge       = 300 * 24 * 60 * 60;      // 8 hour min stake age
...
const string strMessageMagic = "CreditEmperialCoins Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (nHeight <= LAST_POW_BLOCK) // 1500000
        nSubsidy = 8 * COIN;
	*/
	...
	else if (pindexBest->nHeight <= 8000000000) 
        	nSubsidy = 1 * COIN;
	else
		nSubsidy = 0 * COIN;
...
/* 	if (pindexBest->nHeight > (YEARLY_BLOCKCOUNT*37)) // 37 years.
    ...
    if (pindexBest->nHeight <= (10 * YEARLY_BLOCKCOUNT)) // After 2 years, 5% PoS
        nRewardCoinYear = COIN_YEAR_REWARD; 

    int64_t nSubsidy =  0 * nCoinAge * nRewardCoinYear / 365 / COIN;
    //int64_t nSubsidy = 2 * nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CreditEmperialCoins", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "CreditEmperialCoins";
        CTransaction txNew;
        txNew.nTime = 1535369915;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x2e32f7d81827727f10d84d74c7847a6853c8b5004a9c2b54b9c0e38c502365f8"));
        block.print();
